    ...
    """
        ...
        for i in range(count):
            if scores[i] >= self._options.score_threshold:
                ...
                bounding_box = Rect(top=int(y_min * image_height),
                ...
                category = Category(
                    score=scores[i],
                    label=self.
                    _label_list[class_id],  # 0 is reserved for background
                    index=class_id)
        ...
        if self._options.label_allow_list is not None:
            filtered_results = list(
                filter(
                    lambda detection: detection.categories[0].label in self.
                    _options.label_allow_list, filtered_results))
