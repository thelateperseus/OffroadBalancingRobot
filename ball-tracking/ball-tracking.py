...
if not args.get("video", False):
    vs = VideoStream(src=0, usePiCamera=True, resolution=(320,240), framerate=20).start()
...
while True:
    ...
    if len(cnts) > 0:
        ...
        print(f'time: {time.time() - startTime}, centre: {centre}')
        ...
        if radius > 5:
            ...
            panAngle = pidPan.compute(centre[0])
            tiltAngle = pidTilt.compute(240 - centre[1])
