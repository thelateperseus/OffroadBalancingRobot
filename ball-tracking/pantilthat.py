...
class PanTiltHat:
    ...
    def pan(self, angle):
        if angle >= -85 and angle <= 85:
            self.setRotationAngle(1, 85 + angle)
        else:
            print("Angle out of range")

    def tilt(self, angle):
        if angle >= -75 and angle <= 30:
            self.setRotationAngle(0, 75 + angle)
        else:
            print("Angle out of range")
    ...
    def shutdown(self):
        self.write(self.__MODE2,
                   0x00)  #Please use initialization or __MODE2 =0x04
