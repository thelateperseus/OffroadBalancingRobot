...
class PID(object):
    ...
    def compute(self, input_, dt=None):
        """
        ...
        # Avoid derivative kick on first sample
        if self._last_output is None:
            self._derivative = 0
        else:
            self._derivative = -self.Kd * d_input / dt

        print(f'p: {self._proportional}, i: {self._integral}, d: {self._derivative}, error: {error}, dt: {dt}')
